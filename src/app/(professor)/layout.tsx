...
import {
  Home, Users, FolderOpen, ClipboardList, User,
  LogOut, ChevronLeft, ChevronRight, X, Menu,
  Bell, Search, ChevronDown, Megaphone,
} from 'lucide-react'
...
const pageTitles: Record<string, { title: string; subtitle: string }> = {
  '/professor/dashboard': { title: 'Dashboard', subtitle: 'Visão geral das suas turmas' },
  '/professor/turmas': { title: 'Minhas Turmas', subtitle: 'Gerencie disciplinas e alunos' },
  '/professor/comunicados': { title: 'Comunicados', subtitle: 'Envie avisos e informações para os alunos' },
  '/professor/materiais': { title: 'Materiais', subtitle: 'Adicione e organize conteúdos' },
  '/professor/solicitacoes': { title: 'Solicitações', subtitle: 'Revisões de nota e requerimentos' },
  '/professor/perfil': { title: 'Meu Perfil', subtitle: 'Informações do docente' },
}
...
export default function ProfessorLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!loading && user && user.role !== 'PROFESSOR' && user.role !== 'ADMIN') {
  ...
  const initials = (user.professor?.name ?? '').split(' ').map(w => w[0]).slice(0, 2).join('') || 'PR'
  const page = pageTitles[pathname] ?? { title: 'Portal do Professor', subtitle: 'BTCP' }
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full" style={{ background: '#071B34' }}>
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1">
          ...
          {!collapsed && (
            <div className="min-w-0 overflow-hidden">
              ...
              <p className="text-[10px] leading-tight" style={{ color: '#C8A35F' }}>Portal do Professor</p>
            </div>
          )}
        </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        ...
        {navItems.map((item) => {
          const isActive = pathname === item.href || (item.href !== '/professor/dashboard' && pathname.startsWith(item.href))
      ...
      <div className="px-3 pb-4 border-t border-white/8 pt-3 space-y-0.5">
        <button onClick={logout}
          className={cn('w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-red-400/60 hover:text-red-400 hover:bg-red-400/10 transition-all duration-150 group relative', collapsed ? 'justify-center' : '')}>
        ...
        {!collapsed && (
          <div className="mt-2 pt-3 border-t border-white/8 flex items-center gap-3 px-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-white text-xs font-semibold truncate">{user.professor?.name ?? 'Professor'}</p>
              <p className="text-white/30 text-[11px] truncate">{user.professor?.title ?? 'Docente'}</p>
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#f5f6fa]">
      ...
      {/* Mobile sidebar */}
      <aside className={cn('lg:hidden fixed left-0 top-0 h-screen z-50 w-72 transition-transform duration-300 shadow-sidebar', mobileOpen ? 'translate-x-0' : '-translate-x-full')}>
        {sidebarContent}
      </aside>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2.5 ml-1 pl-3 border-l border-gray-100 hover:bg-gray-50 rounded-xl px-2 py-1.5 transition-colors cursor-pointer">
            ...
            <div className="hidden sm:block text-left">
              <p className="text-[13px] font-semibold text-gray-900 leading-none">{user.professor?.name?.split(' ')[0] ?? 'Professor'}</p>
              <p className="text-[11px] text-gray-400 leading-none mt-0.5">Docente</p>
            </div>
